...
double calc_action(const std::complex<double> phi[nmat][nmat]){
  ...
  for(int imat=0;imat!=nmat;imat++){
    for(int jmat=0;jmat!=nmat;jmat++){
      temp=phi2[imat][jmat]*phi2[jmat][imat];
      action=action+0.25e0*temp.real();
    }
  }
  //overall normalization 
  action=action*((double)nmat);
...
double calc_hamiltonian(const std::complex<double> phi[nmat][nmat],
  ...
  for(int imat=0;imat!=nmat;imat++){
    for(int jmat=0;jmat!=nmat;jmat++){
      temp=P_phi[imat][jmat]*P_phi[jmat][imat];
      ham=ham+0.5e0*temp.real();
    }
  }
...
/********************************/
int calc_force(std::complex<double> (&delh)[nmat][nmat],const std::complex<double> phi[nmat][nmat]){
...
int main()
{
  ...
  double ham_init,ham_fin,metropolis,action;
  ...
  for(int iter=0; iter!=niter; iter++){
    ...
    std::cout << ((double)naccept)/((double)iter+1) << ' ' << sum_action/((double)(iter+1)*nmat*nmat) << std::endl;
     
    outputfile << ((double)naccept)/((double)iter+1) << ' ' << sum_action/((double)(iter+1)*nmat*nmat) << std::endl;
